...
# Usage:
#   zsh -c "$(curl -fsLS "https://raw.githubusercontent.com/turboBasic/dotfiles/refs/heads/main/tests/ubuntu/test.zsh?$(date +%s)")"
...
function execute_tests() {
    ...
    (
        ...
        for t in tests/integration/*.sh; do
            ...
            log "▫️ Execute test $test_short_name ($REPO_RAW_URL/$t):"
            ...
            printf '%s %-40s (%s)\n' $result $test_short_name $CHEZMOI_DATA_DIR/$t
        done
    )
}
...
function set_global_vars() {
    ...
    local repo_raw_base="https://raw.githubusercontent.com/$dotfiles_repo"
    local repo_branch="main"
    REPO_RAW_URL="$repo_raw_base/refs/heads/$repo_branch"
...
function install_dotfiles() {
    if [[ "$FLAG_LOCAL" != "true" ]]; then
        source <(curl -fsSL "$REPO_RAW_URL/install.sh?$(date +%s)")
